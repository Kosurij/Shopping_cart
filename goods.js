...
let out = '<div class="table">';
for (let key in cart) {

    out += `<div class="table__item">`;
    out += `<div class="item__inner">`;
    out += `<h2> ${cart[key]['name']}</h2>`;
    out += `<img src='${cart[key]['image']}'>`;
    out += `<p class="price">${cart[key]['price']} RU </p>`;
    out += `<button class="to-cart btn btn-middle" data-articul=${key}>Добавить в корзину</button>`;
    out += `</div>`;
    out += `</div>`;
}
...
const data = {}; // объект с товарами, которые уйдут в корзину
